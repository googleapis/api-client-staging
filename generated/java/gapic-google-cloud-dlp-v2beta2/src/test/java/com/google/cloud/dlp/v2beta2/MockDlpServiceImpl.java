...
import com.google.privacy.dlp.v2beta2.DlpServiceGrpc.DlpServiceImplBase;
...
public class MockDlpServiceImpl extends DlpServiceImplBase {
  ...
  @Override
  public void inspectDataSource(
      InspectDataSourceRequest request, StreamObserver<DlpJob> responseObserver) {
    Object response = responses.remove();
  ...
  public void analyzeDataSourceRisk(
      AnalyzeDataSourceRiskRequest request, StreamObserver<DlpJob> responseObserver) {
    Object response = responses.remove();
    if (response instanceof DlpJob) {
      requests.add(request);
      responseObserver.onNext((DlpJob) response);
