...
use google\monitoring\v3\MetricServiceClient;
 ...
 *     $formattedName = MetricServiceApi::formatProjectName("[PROJECT]");
 *     foreach ($metricServiceApi->listMonitoredResourceDescriptors($formattedName) as $element) {
 *         // doThingsWith(element);
 *     }
 * } finally {
 *     if (isset($metricServiceApi)) {
 *         $metricServiceApi->close();
 *     }
 * }
 * ```
 *
...
class MetricServiceApi
{
    ...
    public function __construct($options = [])
    {
        ...
        $createMetricServiceStubFunction = function ($hostname, $opts) {
            return new MetricServiceClient($hostname, $opts);
        };
     ...
     * try {
     *     $metricServiceApi = new MetricServiceApi();
     *     $formattedName = MetricServiceApi::formatMonitoredResourceDescriptorName("[PROJECT]", "[MONITORED_RESOURCE_DESCRIPTOR]");
     *     $response = $metricServiceApi->getMonitoredResourceDescriptor($formattedName);
     * } finally {
     *     if (isset($metricServiceApi)) {
     *         $metricServiceApi->close();
     *     }
     ...
     *     $metricServiceApi = new MetricServiceApi();
     *     $formattedName = MetricServiceApi::formatProjectName("[PROJECT]");
     *     foreach ($metricServiceApi->listMetricDescriptors($formattedName) as $element) {
     *         // doThingsWith(element);
     *     }
     * } finally {
     *     if (isset($metricServiceApi)) {
     ...
     *     $response = $metricServiceApi->createMetricDescriptor($formattedName, $metricDescriptor);
     * } finally {
     *     if (isset($metricServiceApi)) {
     *         $metricServiceApi->close();
     *     }
     ...
     *     foreach ($metricServiceApi->listTimeSeries($formattedName, $filter, $interval, $view) as $element) {
     *         // doThingsWith(element);
     *     }
     * } finally {
     *     if (isset($metricServiceApi)) {
